...
def get_ll(address):
    ...
    crit_coords = []
    if data:
        ll = list(map(float, data['response']["GeoObjectCollection"]["featureMember"][0]["GeoObject"]["Point"]['pos'].split(' ')))
...
def get_post_code(addres):
    ...
    if data:
        try:
            post_code = data["response"]["GeoObjectCollection"]["featureMember"][0]["GeoObject"]["metaDataProperty"]["GeocoderMetaData"]['Address']["postal_code"]
            if post_code:
                return post_code
